...
def main():
    ap = argparse.ArgumentParser(description="Quick labeling CLI for frames using number keys.")
    ap.add_argument("--csv", required=True, help="Path to labels.csv (columns: filename,label)")
    ap.add_argument("--classes", nargs="*", default=DEFAULT_CLASSES, help="Class names in order of keys 1..N")
    ap.add_argument("--start", type=int, default=None, help="Optional start row index (0-based) among UNLABELED rows")
    ap.add_argument("--autosave-every", type=int, default=1, help="Autosave frequency (every N labels)")
    args = ap.parse_args()
    ...
    while 0 <= pos < len(unlabeled_idx):
        ...
        key_help = " | ".join([f"{i+1}:{name}" for i, name in enumerate(class_names[:5])])
        help_text = f"{key_help}   s:skip   u:undo   q:quit"
        cv2.putText(view, help_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 1, cv2.LINE_AA)

        # Show path at bottom
        cv2.putText(view, os.path.basename(img_path), (10, view.shape[0]-10),
